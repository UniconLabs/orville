...
class IdMatchProcessor implements OpenRegistryProcessor {
    ...
    OpenRegistryProcessorContext process(OpenRegistryProcessorContext processorContext) {
        ...
        response.fullResponse.identifiers.each {
            processorContext.person.addToWallet new TokenIdentifier(systemOfRecord: sor, type: Type.findByTargetAndValue(TokenIdentifier, it.type), token: it.identifier)
        }
        ...
        // set up the outcome body
        if (!processorContext.outcome.body) {
            processorContext.outcome.body = [:]
        }
        processorContext.outcome.body['referenceId'] = response.referenceId
        processorContext.outcome.body.identifiers.addAll(response.fullResponse.identifiers)
