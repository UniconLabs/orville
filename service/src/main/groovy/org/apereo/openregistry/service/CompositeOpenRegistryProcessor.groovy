 ...
 * <code>outcome.isFinal</code> flag of the <code>ProcessorContext</code>. If the end of the processors collection is reached
...
class CompositeOpenRegistryProcessor implements OpenRegistryProcessor {
    ...
    OpenRegistryProcessorContext process(OpenRegistryProcessorContext processorContext) {
        ...
        for (p in this.processingPipeline) {
            ...
            if (ctx.outcome.isFinal) {
                log.info(" Processor '{}' set a 'isFinal' outcome flag signaling the end of processing. Returning processor context: {}", p, ctx)
                return ctx
            }
        }
        log.info("Reached the end of the processing pipeline, but outcome has not been marked as 'final' by any processor. Returning processor context: {}", ctx)
        return ctx
    }
}
